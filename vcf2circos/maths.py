...
def to_arc(interval_theta_array, arc_radius, ideogram_theta_list):
    ...
    try:
        ...
        print("[WARN] *** warning in arc calculation")
        print('[WARN]     interval_theta_array_list:')
        print('[WARN]        '+str(interval_theta_array.tolist()[0:min(max_values_to_show,len(interval_theta_array.tolist()))]))
        print ('[WARN]     assertionError, printing interval_theta_array:')
        print ('[WARN]        '+str(interval_theta_array[np.where(interval_theta_array >= 2*np.pi)][0:min(max_values_to_show,len(interval_theta_array[np.where(interval_theta_array >= 2*np.pi)]))]))
        print ('[WARN]     the problem index starts from:')
