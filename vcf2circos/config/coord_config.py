...
def chr_info(input_file_path, 
    ...
    if type(input_file_path) is dict:
        chr_info_pd = pd.DataFrame.from_dict(data=input_file_path.get('data',{}), orient=input_file_path.get('orient','columns'), columns=input_file_path.get('columns',None))
    # input is a file
    else:
        try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                print('[WARN] unable to read ideogram, printing input file path below')
    ...
    if custom_label:
        try: 
            ...
            print('[WARN] 3rd column is missing for chromosome custom label, using chromosome names as label')
            chr_label = chr_name
    else:
        chr_label = chr_name
    ...
    if custom_spacing:
        try:
            ...
            print('[WARN] 5th column is missing for inter-chromosome custom spacing, using default spacing values')
    ...
    if custom_color:
        try:
            ...
            print('[WARN] 4th column is missing for custom chromosome color, using default method to assign chromosome colors')
            if len(chr_info) <= 30:
...
def data_array(input_file_path, category, chr_info, sep='\t', header='infer', 
    ...
    if type(input_file_path) is dict:
        input_pd = pd.DataFrame.from_dict(data=input_file_path.get('data',{}), orient=input_file_path.get('orient','columns'), columns=input_file_path.get('columns',None))
    else:
        try:
