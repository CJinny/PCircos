...
def chr_info(
    ...
    # input is a dict
    if type(input_file_path) is dict:
        chr_info_pd = pd.DataFrame.from_dict(
            data=input_file_path.get("data", {}),
            orient=input_file_path.get("orient", "columns"),
            columns=input_file_path.get("columns", None),
        )
    # input is a file
    else:
        try:
            ...
            try:
                chr_info_pd = pd.read_csv(
                    input_file_path, sep=sep, header=header, engine="python"
                ).fillna(0)
            except Exception:
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                print(
                                    "[WARN] unable to read ideogram, printing input file path below"
                                )
    ...
    if custom_label:
        try:
            ...
            print(
                "[WARN] 3rd column is missing for chromosome custom label, using chromosome names as label"
            )
            chr_label = chr_name
    else:
        chr_label = chr_name
    ...
    if custom_spacing:
        try:
            ...
            print(
                "[WARN] 5th column is missing for inter-chromosome custom spacing, using default spacing values"
            )
            chr_spacing = np.ones(chr_size.shape) * (
                0.05 * sum(chr_size) / len(chr_size)
            )
    else:
    ...
    if custom_color:
        try:
            ...
            print(
                "[WARN] 4th column is missing for custom chromosome color, using default method to assign chromosome colors"
            )
            if len(chr_info) <= 30:
...
def ideogram_coord_config(
    chr_info,
    npoints=1000,
    show_major_tick=False,
    major_tick_spacing=40000000,
    show_minor_tick=False,
    minor_tick_spacing=20000000,
    show_tick_label=False,
    tick_label_spacing=40000000,
):
    ...
    for i in range(len(ideogram_bin)):
        ideogram_accum_coord_list.append(
            np.concatenate(
                (
                    np.arange(ideogram_bin[i][0], ideogram_bin[i][1] + 1, step),
                    np.arange(ideogram_bin[i][0], ideogram_bin[i][1] + 1, step)[::-1],
                )
            )
        )
    ...
    if show_major_tick:
        ...
        for i in range(len(ideogram_bin)):
            major_tick_accum_coord_list.append(
                np.repeat(
                    np.arange(
                        ideogram_bin[i][0], ideogram_bin[i][1] + 1, major_tick_spacing
                    ),
                    2,
                )
            )
    ...
    if show_minor_tick:
        ...
        for i in range(len(ideogram_bin)):
            minor_tick_accum_coord_list.append(
                np.repeat(
                    np.arange(
                        ideogram_bin[i][0], ideogram_bin[i][1] + 1, minor_tick_spacing
                    ),
                    2,
                )
            )
    ...
    if show_tick_label:
        ...
        for i in range(len(ideogram_bin)):
            tick_label_accum_coord_list.append(
                np.arange(
                    ideogram_bin[i][0], ideogram_bin[i][1] + 1, tick_label_spacing
                )
            )
            tick_label_non_accum_list.append(
                np.arange(0, chr_info["chr_size"][i] + 1, tick_label_spacing)
            )
...
def data_array(
    ...
    if category not in supported_categories:
        raise ValueError(
            "Please choose a supported category, please note that ideogram, ring data does not go through here"
        )

    data_dict = {}

    # input data from dataframe (dict) or file (str)
    if type(input_file_path) is dict:
        input_pd = pd.DataFrame.from_dict(
            data=input_file_path.get("data", {}),
            orient=input_file_path.get("orient", "columns"),
            columns=input_file_path.get("columns", None),
        )
    ...
    if category in ["link", "ribbon", "twistedribbon"]:
        try:
            ...
            print(f'["ERROR"] error, printing input_array in category "{category}"')
            print(input_array)
        ...
        coord = input_array[:, 1:3].astype("float")

        for i in range(len(chr_ideogram_bin)):
            for j in range(len(input_array)):
                if data_chr[j] == chr_info["chr_label"][i]:
                    if category == "histogram":
                        coord[j, :] = coord[j, :] + chr_ideogram_bin[i][0]
