...
class Figure(Complex):
    def __init__(self, *args, **kwargs):
        ...
        if self.config_dict['Category']['ideogram']['patch']['file'].get('dataframe',{}):
            input_data = self.config_dict['Category']['ideogram']['patch']['file'].get('dataframe',None)
        else:
            ...
            elif isfile(os.path.dirname(self.input_json_path)+"/"+self.config_dict['Category']['ideogram']['patch']['file']['path']):
                input_data = os.path.dirname(self.input_json_path)+"/"+self.config_dict['Category']['ideogram']['patch']['file']['path']
        
        if input_data is not None:
            self.chr_info = coord_config.chr_info(input_data, 
            ...
            def single_data_array(key, item):
                ...
                if item['file'].get('dataframe', None) is not None:
                    ...
                    if isfile(item['file']['path']):
                        ...
                        input_data=os.path.dirname(self.input_json_path)+"/"+item['file']['path']
                ...
                if item['file']['header'] in ['None', None, 'none']:
                    return coord_config.data_array(input_data, key,
                                                #self.get_chr_info(), 
                                                self.chr_info,
                                                sep=item['file']['sep'],
                                                header=None,
            ...
            if key != 'highlight':
                ...
                if key == 'cytoband':
                    ...
                    try:
                        ...
                        print (f'[WARN] the column you entered is out of bound, notice that column starts with 0, not 1 ')
                    try:
    ...
    def fig(self):
        ...
        try:
            ...
            return go.Figure(data=sum([*map(lambda t:[t] if not isinstance(t,list) else t, self.trace())], []), layout=self.layout())